...
@section('content')
    ...
    <div style="display:grid; grid-template-columns:1fr 380px; gap:2rem; align-items:start;">
        ...
        <div class="card">
            <div class="card-header">
                <h1 class="card-title" style="display: flex; align-items: center; gap: 0.5rem;">
                    <svg style="width: 22px; height: 22px; fill: none; stroke: var(--primary); stroke-width: 2.5;" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"></path></svg>
                    Thêm sản phẩm mới
                </h1>
            </div>
            ...
            <form action="{{ route('admin.products.store') }}" method="POST" id="productForm">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="VD: Cà phê sữa đá...">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="base_price" id="base_price" value="{{ old('base_price') }}" required min="0" step="1000" placeholder="45000">
                        @error('base_price') <span class="error-text">{{ $message }}</span> @enderror
                    </div>
                </div>

                <div class="form-group">
                    <label for="slug">Slug URL (tự tạo nếu bỏ trống)</label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="ca-phe-sua-da">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="image_url" id="image_url" value="{{ old('image_url') }}"
                           placeholder="Nhấp chọn ảnh từ thư viện bên phải hoặc nhập URL..."
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;" class="mb-3">
                    <div class="form-group" style="margin-bottom:0;">
                        <label class="check-item">
                            <input type="checkbox" name="allow_topping" value="1" {{ old('allow_topping', true) ? 'checked' : '' }}>
                ...
                <button type="submit" class="btn btn-primary" style="width:100%; padding:0.85rem; display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;">
                    <svg style="width: 18px; height: 18px; fill: none; stroke: currentColor; stroke-width: 2.5;" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7"></path></svg>
                    Tạo sản phẩm
                </button>
            </form>
        </div>
        ...
        <div>
            ...
            <div class="card mb-2" style="margin-bottom:1rem;">
                <p class="text-xs text-dim font-bold mb-1" style="text-transform:uppercase; letter-spacing:0.5px; margin-bottom:0.75rem;">Xem trước ảnh</p>
                <div id="img-preview" style="width:100%; aspect-ratio:1; border-radius:0.75rem; overflow:hidden; background:linear-gradient(135deg,#fef3c7,#fed7aa); display:flex; align-items:center; justify-content:center; font-size:3rem; border:2px dashed var(--border);">
                    @if(old('image_url'))
                        <img src="{{ old('image_url') }}" style="width:100%; height:100%; object-fit:cover;">
                    @else
            ...
            @if(!empty($localImages))
                <div class="card" style="padding:1rem;">
                    <p class="text-xs font-bold text-dim mb-1" style="text-transform:uppercase; letter-spacing:0.5px; margin-bottom:0.75rem;">Thư viện ảnh sản phẩm ({{ count($localImages) }} ảnh)</p>
                    <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:0.5rem; max-height:400px; overflow-y:auto; padding-right:0.25rem;">
                        @foreach($localImages as $img)
                            ...
                            <div class="gallery-thumb" onclick="selectImg('{{ $fullUrl }}', this)"
    ...
    <script>
        ...
        // Highlight current image on load
        window.addEventListener('DOMContentLoaded', () => {
            const cur = document.getElementById('image_url').value;
            if (cur) {
                document.querySelectorAll('.gallery-thumb img').forEach(img => {
                    if (img.src === cur) img.closest('.gallery-thumb').classList.add('selected');
                });
            }
        });
    </script>
